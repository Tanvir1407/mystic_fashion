...
import { deleteBrand, restoreBrand } from "../catalog-actions";
            ...
            {brands.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-12 text-center text-sm text-slate-500 font-medium">
                  No {currentTab === "trash" ? "deleted" : ""} brands found.
                </td>
              </tr>
            ) : (
              brands.map((brand) => (
                <tr key={brand.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 text-xs font-bold ${brand.active && !brand.deletedAt ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}>
                      {brand.deletedAt ? "Deleted" : brand.active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
