...
export default async function AdminDiscountsPage() {
  ...
  const canView = hasPermission(session, "VIEW", "DISCOUNTS");
  const canCreate = hasPermission(session, "CREATE", "DISCOUNTS");
  const canEdit = hasPermission(session, "EDIT", "DISCOUNTS");
  const canDelete = hasPermission(session, "DELETE", "DISCOUNTS");
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <div className="w-full max-w-md bg-white dark:bg-zinc-900 shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-slate-100 dark:border-zinc-800 rounded-xl p-8 md:p-10 text-center flex flex-col items-center animate-fade-in">
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mb-2 leading-relaxed max-w-xs font-normal">
            You do not have permission to view discounts. Please contact your administrator to assign permissions for your working area.
          </p>
        </div>
      </div>
    );
