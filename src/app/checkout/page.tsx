...
export default function CheckoutPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Custom validation
    if (!formData.get("fullName") || !formData.get("phone") || !formData.get("district") || !formData.get("address")) {
              ...
              <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm sticky top-32">
                ...
                <div className="border-t border-slate-100 pt-4 space-y-3 mb-6">
                  ...
                  <div className="flex justify-between text-zinc-600 text-sm">
                    ...
                    <span className="font-bold text-zinc-900">{formatBDT(deliveryFee)}</span>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  form="checkout-form"
                  className="w-full bg-primary text-white py-4 rounded-xl font-black uppercase tracking-widest hover:bg-[#600018] transition-all transform active:scale-[0.98] shadow-lg shadow-black/10 flex items-center justify-center gap-2"
                >
                  Place Order
                </button>
              </div>
