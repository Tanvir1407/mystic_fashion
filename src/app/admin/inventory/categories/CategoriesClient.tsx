...
export default function CategoriesClient({
  ...
  const [displayEditId, setDisplayEditId] = useState<string | null>(null);
  const [uploading, setUploading] = useState<string | null>(null);
  const [editData, setEditData] = useState<{ image: string | null; sortOrder: number }>({
    image: null,
    sortOrder: 0,
  });
  ...
  const startDisplayEdit = (cat: any) => {
    setDisplayEditId(cat.id);
    setEditData({
  ...
  const handleSaveDisplay = (categoryId: string) => {
    startSaveDisplayTransition(async () => {
      const res = await updateCategoryDisplay(categoryId, {
      ...
      setLocalCategories((prev) =>
        prev.map((c) =>
          c.id === categoryId
            ? { ...c, image: editData.image, sortOrder: editData.sortOrder }
            : c
        )
      );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedCategories.length === 0 ? (
          ...
          sortedCategories.map((category) => {
            const resolvedImage =
              category.image || FALLBACK_IMAGES[category.name] || null;
            const isEditingDisplay = displayEditId === category.id;
                            ...
                            <>
                              <button
                                onClick={() =>
                                  isEditingDisplay
                                    ? setDisplayEditId(null)
                                    : startDisplayEdit(category)
                                }
                                title="Edit Image & Display Order"
                                className={`p-1.5 rounded border text-xs transition-all ${
                                  isEditingDisplay
                                    ? "bg-indigo-100 border-indigo-300 text-indigo-700"
                                    : "bg-slate-50 border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50"
                                }`}
                              >
                  ...
                  {isEditingDisplay && (
                    <div className="mt-3 pt-3 border-t border-indigo-100 space-y-3">
                      ...
                      <div>
                        ...
                        {/* Preview */}
                        <div className="relative w-full aspect-[3/2] bg-slate-100 rounded overflow-hidden border border-slate-200 mb-2">
                          {editData.image ? (
                            <>
                              <Image
                                src={editData.image}
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 text-slate-400">
                              ...
                              <span className="text-[10px]">No image — fallback will be used</span>
                            </div>
                          )}
                        </div>

                        {/* Upload button */}
                        <label className="flex items-center justify-center gap-2 w-full cursor-pointer text-center px-3 py-2 border border-slate-300 rounded text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-colors bg-white">
                          {uploading === category.id ? (
                            ...
                            <>
                              ...
                              {editData.image ? "Replace Image" : "Upload Image"}
                            </>
                          )}
                          <input
                            type="file"
                            ...
                            onChange={(e) => {
                              ...
                              if (f) handleImageUpload(f, category.id);
                            }}
                          />
                        </label>
                        <p className="text-[10px] text-slate-400 mt-1">
                          Recommended: 3:4 portrait ratio (e.g. 600×800 px). If no image is set, the default fallback image will be shown.
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-28 text-sm px-3 py-2 border border-slate-200 rounded focus:outline-none focus:border-indigo-400 font-mono"
                        />
                        <p className="text-[10px] text-slate-400 mt-1">
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          onClick={() => handleSaveDisplay(category.id)}
                          disabled={isSavingDisplay || !!uploading}
                          className="flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded hover:bg-slate-700 transition disabled:opacity-60"
                        >
                          <Save className="w-3 h-3" />
                          {isSavingDisplay ? "Saving..." : "Save"}
                        </button>
                        <button
                          onClick={() => setDisplayEditId(null)}
                          className="px-3 py-2 border border-slate-200 text-slate-600 text-xs font-semibold rounded hover:bg-slate-50 transition"
                        >
                          Cancel
                        </button>
                      </div>
